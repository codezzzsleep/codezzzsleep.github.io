...
1. 授予用户权限
   ...
   -- 如果加上 with grant option 则代表用户可以继续把自己拥有的权限授予别人
   ```
...
3. 数据库角色
   ...
   3. 角色权限收回
      ...
      ```sql
      revoke select on table_name from <角色名>
      ```
   ...
   4. 把角色授予用户或角色
      ...
      ```sql
      grant <角色> to user1 [with grant option]
      ```

4. 视图机制

   1. 为不同的用户定义不同的视图，把不需要的视图隐藏起来，这样用户就不会误操作

5. 审计

   ![image-20230207001410877](../../../../resource/image-20230207001410877.png)
